		...
		for instrument in self.soundBank['instruments']:
			for group in instrument['groups']:
				for region in group['regions']:
					...
					samplePath = sample
					logging.info("Original sample path: {}".format(sample))
					logging.info("SoundBank Path: {}".format(self.soundBank.get('Path', 'None')))
					
					if not os.path.isabs(samplePath):
						if 'Path' in self.soundBank.keys() and self.soundBank['Path']:
							samplePath = os.path.join(self.soundBank['Path'], sample)
							logging.info("Joined path: {}".format(samplePath))
						# If no path or empty path, try current directory
						if not os.path.exists(samplePath):
							samplePath = sample
							logging.info("Using original sample path: {}".format(samplePath))
					
					logging.info("Final sample path: {}".format(samplePath))
					logging.info("File exists: {}".format(os.path.exists(samplePath)))
					
					try:
						data, rate = soundfile.read(file=samplePath, dtype='int16', always_2d=True)
					except Exception as e:
						logging.error("Can not read input audio file {}: {}".format(samplePath, str(e)))
						# Try alternative methods for malformed WAV files
						try:
							# Try with pydub using raw file reading for ADPCM
							from pydub import AudioSegment
							import numpy as np
							
							logging.info("Trying to read ADPCM file with pydub...")
							# Read the file directly without conversion
							audio = AudioSegment.from_file(samplePath, format="wav")
							...
							logging.info("Successfully read ADPCM file - Rate: {}, Channels: {}, Samples: {}".format(rate, audio.channels, len(samples)))
							
						except ImportError:
							logging.error("pydub not available. Please install with: pip install pydub")
							raise SF2ExportError
						except Exception as pydub_e:
							logging.error("pydub also failed: {}".format(str(pydub_e)))
							# Try one more approach with scipy if available
							try:
								from scipy.io import wavfile
								logging.info("Trying with scipy.io.wavfile...")
								rate, data = wavfile.read(samplePath)
								if len(data.shape) == 1:
									data = data.reshape((-1, 1))
								logging.info("Successfully read with scipy - Rate: {}, Shape: {}".format(rate, data.shape))
							except ImportError:
								logging.error("scipy not available either")
								raise SF2ExportError
							except Exception as scipy_e:
								logging.error("scipy also failed: {}".format(str(scipy_e)))
								# Final attempt: use ffmpeg directly to convert to PCM
								try:
									import subprocess
									import tempfile
									
									logging.info("Trying direct ffmpeg conversion...")
									# Create temporary PCM file
									with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_file:
										temp_path = temp_file.name
									
									# Use ffmpeg to convert ADPCM to PCM
									cmd = ['ffmpeg', '-y', '-i', samplePath, '-acodec', 'pcm_s16le', temp_path]
									result = subprocess.run(cmd, capture_output=True, text=True)
									
									if result.returncode == 0:
										# Now try to read the converted file
										try:
											data, rate = soundfile.read(file=temp_path, dtype='int16', always_2d=True)
											logging.info("Successfully converted and read with ffmpeg - Rate: {}, Shape: {}".format(rate, data.shape))
											# Clean up temp file
											os.unlink(temp_path)
										...
										os.unlink(temp_path)
										logging.error("ffmpeg conversion failed: {}".format(result.stderr))
										raise SF2ExportError
								except FileNotFoundError:
									logging.error("ffmpeg not found in PATH")
									raise SF2ExportError
								except Exception as ffmpeg_e:
									logging.error("ffmpeg conversion failed: {}".format(str(ffmpeg_e)))
									raise SF2ExportError
					channels = len(data[0])
					if channels < 1:
